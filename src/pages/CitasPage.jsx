...
export default function CitasPage() {
  ...
  const citasFiltradas = useMemo(() => {
    ...
    return [...lista].sort((a, b) => {
      if (orden.campo === "fecha") {
        return factor * `${a.fecha}${a.hora}`.localeCompare(`${b.fecha}${b.hora}`);
      }
      if (orden.campo === "motivo") {
        return factor * a.motivo.localeCompare(b.motivo);
      }
      return 0;
    });
  }, [citas, filtroEstado, orden]);
  ...
  const { page, setPage, totalPages, pageItems, totalItems, pageSize } = usePagination(citasFiltradas, 8);
  ...
  return (
    <div>
      ...
      {confirmTarget && (
        <ConfirmDialog
          title="Eliminar cita"
          message="¿Seguro que quieres eliminar esta cita del sistema? Esta acción no se puede deshacer."
